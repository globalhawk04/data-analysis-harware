...
# --- Define the data to be plotted and analyzed (Comparison Period - Coffee) ---
...
for i, col in enumerate(cols_to_plot):
    ...
    if perform_analysis:
        try:
            ...
            if order is not None and len(y) > max(order) * 2: # Need sufficient total data points for fitting
                 ...
                 # Using 'basinhopping' may help find better fit for complex models,
                 # but can be slower. Default 'lbfgs' is often sufficient.
            ...
            analysis_results[col] = {'error': str(e)} # Store error message
    ...
    if col in analysis_results and 'error' not in analysis_results[col]:
    ...
    elif col in analysis_results and 'error' in analysis_results[col]:
         annotation_text = f"Analysis Error:\n{analysis_results[col]['error']}"
         ax.text(0.98, 0.98, annotation_text, transform=ax.transAxes, fontsize=8, color='red',
                verticalalignment='top', horizontalalignment='right',
                bbox=dict(boxstyle='round,pad=0.3', fc='white', alpha=0.7))
...
# Save the figure
plt.savefig('cow_learn_4_analysis.png') # Changed filename again
