...
plot_start_str = baseline1_start_str # Define the overall plot start
plot_end_str = baseline2_end_str   # Define the overall plot end
...
segments = [
    (baseline1_start_str, baseline1_end_str, 'lightgray', 'Baseline 1'),
    (hay_start_str, cowpatty_start_str, 'lightgreen', 'Hay Test'),
    (cowpatty_start_str, dirt_start_str, 'burlywood', 'Cowpatty Test'),
    (dirt_start_str, grass_start_str, 'saddlebrown', 'Dirt Test'),
    (grass_start_str, farm_end_str, 'olivedrab', 'Grass Test'),
    (baseline2_start_str, baseline2_end_str, 'lightgray', 'Baseline 2')
    # Note: Gaps between segments are left uncolored by default
]
...
fig.suptitle('Sensor Readings vs. Baseline Averages with Test Segments', fontsize=16)
...
for i, col in enumerate(plot_df.columns):
    ...
    # --- Plot the Baseline Average Lines (zorder=4) ---
...
handles, labels = [], []
# Use a set to track added labels and ensure uniqueness
added_labels = set()
for ax in axes:
    # Get handles and labels from axvspan (patches) and plots/hlines (lines)
...
# Adjust ncol and bbox_to_anchor as needed based on the number of legend entries
fig.legend(handles, labels, loc='lower center', ncol=5, bbox_to_anchor=(0.5, 0))


# Save the figure
plt.savefig('cow_learn_gas_segments.png') # Changed filename again
